...
def transform_step_logits(base_model_tokenizer: transformers.tokenization_utils_base.PreTrainedTokenizerBase,
    ...
    base_model_special_token = get_special_token_marker(base_model_tokenizer)
    blending_model_special_token = get_special_token_marker(blending_model_tokenizer)
...
#把一个 SLM 的 logits dataset 对齐到 LLM 的 logits dataset 上（token 粒度一致化），并返回融合后的 dataset
#把 SLM 的 logits 翻译成 LLM 的 token 空间，附加到 dataset 上。
def token_align(
    base_model_logits_datasets,       # 基准模型（LLM）的 logits 数据集
    blending_model_logits_dataset,    # 待融合模型（SLM）的 logits 数据集
    base_tokenizer,                   # LLM 的 tokenizer
    blending_tokenizer,               # SLM 的 tokenizer
    blending_to_base_mapping,         # SLM → LLM 的词表映射
    blending_model_index,             # 标记是第几个 SLM
    batch_size=4,                     # 批大小
    preprocessing_num_workers=4,      # 预处理进程数
    skip_align=False,                 # 是否跳过对齐
    align_strategy="dtw",             # 对齐策略 (默认 DTW: 动态时间规整)
):
